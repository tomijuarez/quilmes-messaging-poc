module.exports.processEvent = function (context, req) {
    ...
    parsedReq.forEach(rawEvent => {
        ...
        if (rawEvent.eventType == subscriptionValidationEvent) {
            ...
            if (eventPayload.employeeId % 2 === 0) {
                context.log(`Event ${eventPayload.eventId} received and processed :)`);
                context.res = {
                    status: 200,
                    body: `Event ${rawEvent.eventId} received and processed :)`,
                    headers: { 'Content-Type': 'application/json' }
                };
            }
            else {
                context.log(`Event ${eventPayload.eventId} received but it could not be processed :(`);
                context.res = {
                    status: 500,
                    body: `[sometimesFailsEmployeeEventSubscriber]: Event ${rawEvent.eventId} received but it could not be processed :(`
                };
            }
        } else {
            context.log('>:v');
